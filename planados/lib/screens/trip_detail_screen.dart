...
class _TripDetailScreenState extends State<TripDetailScreen> {
  late Map<int, List<String>> dayActivities;
  ...
  Future<void> _loadOrInitializeActivities() async {
    try {
      ...
      if (snapshot.exists) {
        ...
        // Find trip by matching title and destination
        tripsData.forEach((key, value) {
          final tripData = value as Map<dynamic, dynamic>;
          if (tripData['title'] == widget.trip.title &&
              tripData['destination'] == widget.trip.destination) {
            _tripId = key as String;

            // Load existing activities if available
            if (tripData['activities'] != null) {
              final activitiesData =
                  tripData['activities'] as Map<dynamic, dynamic>;
              dayActivities = {};
    ...
    // Initialize empty activity lists for days without activities
    dayActivities ??= {};
